...
var _objectAssign$2 = typeof Object.assign === 'function' ? Object.assign : _objectAssign$1;
 ...
 */
var set$1 = /*#__PURE__*/_curry3(function set(lens, v, x) {
...
var authenticate = (function (api) {
  var auth = {};
  var assignKey = curry(function (key, ref, val) {
    ref[key] = val;
    return ref;
  });
  ...
  var headerType = flip(prop)(constants.headers);
  var addClaimToRequest = curry(function (type$$1, requestOptions, claim) {
    return assocPath(['headers', constants.headerPrefix + headerType(type$$1)], claim.accessToken, requestOptions);
  });
  ...
  var setClaim = curry(function (type$$1, auth) {
    return compose(assignKey(type$$1, auth), evolve({
  ...
  var getClaim = curry(function (type$$1, auth) {
    return pathOr({}, [type$$1], auth);
  });
  ...
  var passwordPrompt = function () {
    ...
    return function prompt(requestOptions) {
      return inquirer.prompt([{
        ...
        return getUserAccessToken(assocPath(['developerAccount', 'password'], password, api.context));
      }).then(compose(setUserClaim(auth), prop('data'))).then(compose(addUserClaimToRequest(requestOptions), getUserClaim)).catch(function (e) {
  ...
  var authenticate = function authenticate(requestOptions) {
    ...
    if (isServer) {
      if (requestOptions.scope !== constants.dev && (appClaim.accessToken || requestOptions.context.sharedSecret)) {
        ...
        return appClaim.stale ? refreshAppAccessToken(appClaim) : getAppAccessToken(api.context).then(compose(setAppClaim(auth), prop('data'))).then(compose(addAppClaimToRequest(requestOptions), getAppClaim));
      }
...
var api = {
  setContext: function setContext() {
    ...
    var normalizedContextKeys = reduce(function (keyMap, key) {
      keyMap[key.replace(/[^0-9a-zA-Z]/g, '').toUpperCase()] = key;
      return keyMap;
    }, {}, keys(context)); //set default headers

    this.headers = reduce(function (headers, _ref) {
      var _ref2 = _slicedToArray(_ref, 2),
          key = _ref2[0],
          name = _ref2[1];

      var value = context[normalizedContextKeys[key] || key] || headerDefaults[key];
      if (value) headers[constants.headerPrefix + name] = value;
      return headers;
    }, {}, toPairs(constants.headers));
    api.methods = values(constants.verbs).map(function (v) {
      return v.toLowerCase();
    });

    if (CONSTANTS.isServer) {
      api.headers['User-Agent'] = "K Isomorphic SDK v".concat(constants.version, " (Node.js ").concat(process.version, "; ").concat(process.platform, " ").concat(process.arch, ")");
    }

    this.hookMap = CONSTANTS.hooks.reduce(function (acc, hookName) {
      return set$1(lensProp(hookName), ifType('Function', options.hooks[hookName], function (x) {
        return x;
      }), acc);
    }, {});
    Object.assign(this, {
...
api.parseTemplate = function () {
   ...
   */
  var usedKeys = [];
  var url = compose(join('?'), addIndex(map)(function (l, x) {
    return x ? l.filter(function (v) {
      return v[1];
    }).map(function (m) {
      return m.join('=');
    }).join('&') : l;
  }), addIndex(map)(function (c, x) {
    return x ? c.split('&').map(function (q) {
      return q.split('=');
    }) : c;
  }), split('?'), replace(constants.templateBraceRegex, function (str) {
...
api.request = curryN(4, function () {
  ...
  if (CONSTANTS.isServer) {
    var config = mergeDeepRight({
      ...
      method: method,
      url: url
    }, omit(['context'], requestOptions)); // console.log(api.headers)

    if ([constants.verbs.POST, constants.verbs.PUT].map(function (str) {
      return str.toLowerCase();
    }).includes(method.toLowerCase())) {
      if (requestOptions.body) config.data = requestOptions.body;else config.data = omit(usedKeys, data);
    }
...
var uncompressedServices = map(compose(replace('~', ''), join(''), map(function (piece) {
  return propOr(piece, piece, templateReference);
}), splitWhenStr(test(constants.urlTemplateSplitRegex))), uncompressFlattened(definition, CONSTANTS.delimiter)); //client tree

var services = crawl(function (obj, key, path$$1) {
  ...
  var args = match(constants.templateBraceRegex, route).map(function (match$$1) {
    var innerMatch = match$$1.substring(1, match$$1.length - 1);
    return innerMatch[0] === '+' ? '' : innerMatch;
  }).filter(Boolean);
  var params = match(constants.templateBraceRegex, query).map(function (match$$1) {
    return match$$1.substring(1, match$$1.length - 1);
  });
  var request = api.request(path$$1, method, url); //store useful information on the method
...
api.auth = authenticate(api); //#endregion expand and build service tree / methods
